...
class GetNewsBySelectedCategories @Inject constructor(
    private val repo: NewsRepository, private val getSelectedCategories: GetSelectedCategories
) {
    suspend operator fun invoke(): Flow<Resource<List<Article>>> {
        return repo.getTopHeadlinesBySelectedCategories(getSelectedCategories())
    }
}